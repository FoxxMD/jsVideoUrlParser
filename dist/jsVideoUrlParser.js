(function (global, factory) {
  ...
  var getQueryParams$1 = function getQueryParams(qs) {
    if (typeof qs !== 'string') {
      return {};
    }
    qs = qs.split('+').join(' ');
  ...
  var combineParams$e = function combineParams(params, hasParams) {
    if (_typeof(params) !== 'object' || Object.keys(params).length === 0) {
      return '';
    }
    var urlParams = new URLSearchParams(params);
    urlParams.sort();
    var combined = urlParams.toString();
    return "".concat(hasParams === true ? '&' : '?').concat(combined);
  };
  ...
  var getTime$6 = function getTime(timeString) {
  ...
  Dailymotion.prototype.createUrl = function (base, vi, params) {
    ...
    return base + vi.id + combineParams$b(params);
  };
